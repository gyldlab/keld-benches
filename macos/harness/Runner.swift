...
private func runSelfTests(html: Data) async throws {
    ...
    try require(try rawHTTPStatus(port: server.port, target: beaconPrefix + "client_now_ms=12&script_start_ms=1&raf1_ms=10&raf2_ms=8&visibility=visible&focus=true") == 422, "unordered diagnostic must be rejected")
    try require(server.events(for: second).last?.reason == "unordered rendering-opportunity diagnostics", "unordered rejection reason must remain precise")
