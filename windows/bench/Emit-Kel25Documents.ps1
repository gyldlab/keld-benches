...
Write-NoBom (Join-Path $BenchRepo 'windows\bench\results\mem-idle\2026-08-24.kel25-windows-fixture-idle-30.fresh-process.json') ([ordered]@{
  ...
  session=[ordered]@{ started_utc=$startUtc; finished_utc=$finishUtc; requested_samples=30; interleaving='none'
    ...
    notes='Idle RSS of the COMMITTED windows/keld/hello fixture through the shipping keld dev path, 30 fresh-process runs (the gui-paint-rss sample-policy count). SCORED value is the Keld-owned main-process (keld.exe) working set per metrics.v1.json MEM-IDLE. Every other lane is a per-run diagnostic and is never blended: bun child, WebView2 engine helpers, keld_processes_ws_kib (host+bun, the scope architecture 01 section 5 uses for its <=92160 KiB budget), and the whole descendant tree. Stability requires BOTH bounded drift (6 consecutive tree totals within 1% of the window mean) AND an identical process-class census across that window with a non-zero engine count. The census condition was added after a 30-run session produced one byte-stable but INCOMPLETE tree (55,336 KiB, engine_process_count=0, against a ~372,000 KiB median) that drift alone accepted; adding it also cut host-lane stdev from 986 to 69 KiB and engine-lane stdev from 59,139 to 3,376 KiB, so membership churn had been contaminating many samples rather than one. Raw run records: results/mem-idle/2026-08-24.kel25-windows-fixture-idle-30.fresh-process.raw.json. BUDGET READING, RESOLVED FROM THE SPEC'S OWN WORDS. architecture 01 section 5 states the budget as "Idle RSS, 1 window (sum of keld processes) <= 90 MB". RSS is RESIDENT set size; on Windows the resident quantity is the WORKING SET. Private commit is not RSS -- it counts reserved pages that are by definition not resident -- and metrics.v1.json's own oracle already separates them: "Keld-owned RSS, engine helpers, total coalition/tree, and private dirty reported separately". Private dirty is named there as a separate diagnostic, never the scored value. On the working-set counter BOTH defensible scopes pass: host only 22,704 KiB (PASS, +75% headroom) and host+bun 45,140 KiB (PASS, +51%). For completeness the private-commit lane is also recorded per run (host+bun 92,332 KiB, which would exceed the budget) precisely so no reader has to take the counter choice on trust -- but it is a diagnostic, not the score, and reporting it as a budget failure would contradict both the architecture wording and the registry oracle. The one genuine residual ambiguity is SCOPE: the budget says "sum of keld processes" (host+bun) while the registry note says "the Keld-owned (main) RSS" (host only). Both pass; the registry should still say which. NOT a budget verdict for the additional usual reasons: single arm, not interleaved, thermal state unknown.' }
  environment=(New-Env)
  ...
  publication=[ordered]@{ policy_version=1; requested=$false; eligible=$false
    reasons=($COMMON + @(
      [ordered]@{ code='REGISTRY_SCOPE_INCONSISTENT'; label='architecture 01 section 5 budgets the "sum of keld processes" (host+bun) while the metrics.v1.json note directs scoring "the Keld-owned (main) RSS" (host only). Both readings pass on the working-set counter (45,140 and 22,704 KiB against 92,160), so this does not change the verdict, but the registry should still state which scope is scored' })) } })
